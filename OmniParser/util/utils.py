...
site_packages = os.path.join(os.path.dirname(os.path.dirname(__file__)), ".venv", "Lib", "site-packages")
for dll_dir in (
...
PADDLEOCR_PYTHON = os.environ.get(
    "OMNIPARSER_PADDLEOCR_PYTHON",
    os.path.join(os.path.expanduser("~"), ".conda", "envs", "omni-paddleocr", "python.exe"),
)


def run_paddleocr_worker(image_source: Image.Image, text_threshold: float):
    worker_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "tools", "paddleocr_worker.py")
    if not os.path.exists(PADDLEOCR_PYTHON):
        raise RuntimeError(f"PaddleOCR environment Python not found: {PADDLEOCR_PYTHON}")
    ...
    try:
        ...
        if not lines:
            raise RuntimeError(completed.stderr.strip() or "PaddleOCR worker produced no JSON output")
        payload = json.loads(lines[-1])
        return payload["coord"], payload["text"]
    finally:
        try:
            os.remove(tmp_path)
        except OSError:
...
def check_ocr_box(image_source: Union[str, Image.Image], display_img = True, output_bb_format='xywh', goal_filtering=None, easyocr_args=None, use_paddleocr=False):
    ...
    if use_paddleocr:
        ...
        try:
            coord, text = run_paddleocr_worker(image_source, text_threshold)
        except Exception as exc:
            print(f"PaddleOCR failed, falling back to EasyOCR: {type(exc).__name__}")
            use_paddleocr = False
