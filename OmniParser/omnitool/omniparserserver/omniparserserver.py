...
def format_parsed_content_list(parsed_content_list):
    formatted = []
    for item in parsed_content_list:
        ...
        if "bbox" in item:
            formatted_item["bbox"] = item.get("bbox")
        if "idx" in item:
            formatted_item["idx"] = item.get("idx")
        for key, value in item.items():
            if key in {"content", "bbox", "idx", "type", "interactivity", "source"}:
                continue
...
async def parse(parse_request: ParseRequest):
    ...
    return {
        ...
        "parsed_content_list": format_parsed_content_list(parsed_content_list),
        'latency': latency,
    }
